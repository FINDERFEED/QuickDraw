...
import com.finderfeed.quick_draw.init.QDCoreShaders;
...
public class RadialMenu {
    ...
    public void drawMenu(GuiGraphics graphics,int mouseX,int mouseY,float pTicks,float zOffset,int section){
        ...
        RenderSystem.setShader(()-> QDCoreShaders.RADIAL_MENU);
        settings.applyUniforms(QDCoreShaders.RADIAL_MENU,section);

        BufferBuilder builder = Tesselator.getInstance().begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);

        builder.addVertex(m,centerX - radius,centerY - radius,zOffset).setUv(0,0);
        builder.addVertex(m,centerX - radius,centerY + radius,zOffset).setUv(0,1);
        builder.addVertex(m,centerX + radius,centerY + radius,zOffset).setUv(1,1);
        builder.addVertex(m,centerX + radius,centerY - radius,zOffset).setUv(1,0);
